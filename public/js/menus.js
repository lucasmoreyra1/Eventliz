...
$(document).ready(function() {
    $('#agregarMenu').on('show.bs.modal', function (e) {
        var modal = $(this);
        $.ajax({
            url: "/menus/create",
            method: 'GET',
    ...
    $('#saveMenuBtn').on('click', function() {
        var form = $('#agregarMenu').find('form');
        $.ajax({
    ...
    $('#editarMenu').on('show.bs.modal', function (e) {
        var modal = $(this);
        ...
        if (filaSeleccionada.length > 0) {
            ...
            $.ajax({
                url: "/menus/" + eventoId + "/edit", // Ajusta la URL según tu ruta de edición
                method: 'GET',
                success: function(response) {
                    modal.find('.modal-body').html(response);
                },
    ...
    $('#editarMenuBtn').on('click', function() {
        var form = $('#editarMenu').find('form');
        $.ajax({
            url: form.attr('action'),
            method: form.attr('method'),
            data: form.serialize(),
...
$('#eliminarMenuBtn').on('click', function() 
{
    var filaSeleccionada = $('.evento-row.table-primary');
        ...
        if (filaSeleccionada.length > 0) {
            ...
            // Realizar la petición AJAX pasando el ID del evento al servidor
            $.ajax({
                url: "menus/" + eventoId, // Ajusta la URL según tu ruta de edición
                method: 'DELETE',
                headers: {
